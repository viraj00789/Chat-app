...
const StatusSideBar = () => {
  ...
  const handleUserStatuses = (userId) => {
    setUserStatuses((prev) => {
      ...
      const nextUserIndex = viewed.findIndex((user) => user.id === userId) + 1;
      const nextUser = viewed[nextUserIndex] || null;
      ...
      if (!prevViewed.some((view) => view.id === userId)) {
        console.log("inn","in user status");
  ...
  const handleIndexes = (userId, newIndex) => {
    setUserStatuses((prev) => {
      ...
      if (newIndex === updatedUserStatuses[userId]?.images?.length - 1) {
        console.log("inno");
        const nextUserIndex = viewed.findIndex((user) => user.id === userId) + 1;
        const nextUser = viewed[nextUserIndex] || null;
  ...
  return (
    <>
      ...
      <UserStatus
        ...
        handleSelecedItem={handleSelecedItem}
